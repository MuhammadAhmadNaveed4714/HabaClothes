...
export default function AdminEditProductPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const resolved = await productsApi.getById(id);
        if (!active) return;
        setProduct(resolved);
        setName(resolved.name || "");
        setDescription(resolved.description || "");
        setCategory(resolved.category || "");
        setPrice(String(resolved.price ?? ""));
        setStock(String(resolved.stock ?? ""));
        setSizes((resolved.sizes || []).join(", "));
        setColors((resolved.colors || []).join(", "));
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="font-display text-4xl font-light">Update Product</h1>
        <p className="font-mono text-xs text-ink/40 mt-1 tracking-widest">Edit catalog details</p>
      </div>
      ...
      {loading ? (
        <div className="border border-bone bg-chalk px-6 py-8 font-mono text-xs text-ink/40">
          Loading product...
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex items-center justify-between">
            <p className="font-mono text-xs text-ink/40 tracking-widest">
              {saving ? "Saving product..." : "All fields are saved on submit"}
            </p>
            <button
              type="submit"
              disabled={saving}
